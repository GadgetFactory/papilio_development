...
RUN apt-get update \
    ...
    && wget -P /tmp/ https://cdn.gowinsemi.com.cn/Gowin_V1.9.9.03_Education_linux.tar.gz \
...
# Install litex
RUN apt-get update \
    && apt-get install -y python3-full python3-pip git bash-completion gcc-riscv64-linux-gnu meson vim \
...
# Install apio
RUN . /usr/local/share/litex/venv/bin/activate \
    && pip install apio
